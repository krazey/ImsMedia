...
TEST_F(RtpHeaderExtensionTest, TextConstructor)
{
    RtpHeaderExtension extension1;
    EXPECT_EQ(extension1.getExtensionData(), nullptr);
    EXPECT_EQ(extension1.getLocalIdentifier(), 0);
    EXPECT_EQ(extension1.getExtensionDataSize(), 0);

    RtpHeaderExtension* extension2 = new RtpHeaderExtension(*extension);
    EXPECT_EQ(memcmp(extension2->getExtensionData(), kExtensionData, kExtensionDataSize), 0);
    EXPECT_EQ(extension2->getLocalIdentifier(), kIdentifier);
    EXPECT_EQ(extension2->getExtensionDataSize(), kExtensionDataSize);
    delete extension2;
}
...
{
    Parcel parcel;
    EXPECT_EQ(extension->writeToParcel(nullptr), BAD_VALUE);
    EXPECT_EQ(extension->writeToParcel(&parcel), NO_ERROR);
    ...
    EXPECT_EQ(extension2->readFromParcel(nullptr), BAD_VALUE);
    EXPECT_EQ(extension2->readFromParcel(&parcel), NO_ERROR);
...
TEST_F(RtpHeaderExtensionTest, TestNotEqual)
{
    ...
    extension2->setExtensionData(kExtensionData, kExtensionDataSize);
    extension2->setLocalIdentifier(9999);
    extension2->setExtensionDataSize(kExtensionDataSize);
